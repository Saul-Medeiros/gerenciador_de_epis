    ...
    <body>
        <div class="main-container d-flex">
            ...
            <div id="conteudo" class="bg-background" >
                ...
                <div class="container">
                    <h3 class="text-center">Listagem de Alocações</h3>
                    <a href="cadastrarAlocacao.jsp"
                    ...
                    <table class="table table-hover table-striped"
                        ...
                        <tbody>
                            <c:forEach items= "${alocacoes}" var="a">
                                <tr>
                                    <td>${a.idAlocacao}</td>
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${a.dataDevolucao != null}">
                                                <fmt:formatDate pattern="dd/MM/yyyy" 
                                                    value="${a.dataDevolucao}"/>
                                            </c:when>    
                                            <c:otherwise>
                                                Devolução não definida
                                            </c:otherwise>
                                        </c:choose>
                                    </td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${a.dataDevolucao == null}">
                                                <a href="gerenciarAlocacao?acao=alterarDevolucao&idAlocacao=${a.idAlocacao}"
                                                   class="btn btn-warning btn-sm"
                                                   role="button">
                                                   <i class="bi bi-calendar-date"></i> Definir data de devolução
                                                </a>
                                            </c:when>
                                        </c:choose>

                                        <a href="gerenciarAlocacaoEpi?acao=alocar&idAlocacao=${a.idAlocacao}"
                                           class="btn btn-success btn-sm"
                                           role="button">
                                           <i class="bi bi-box-arrow-in-down"></i> Alocar Nova Epi
                                        </a>
                                    </td>
                                </tr>
                            </c:forEach>
